...
| `next-change` / `prev-change`                            | jump to the next / previous changed line    | `}` / `{`                                   | —                             |
...
`next-hunk` / `prev-hunk` keep their canonical config IDs and step between **change runs**, not hunks in user-facing copy. Their in-pane defaults are `]` and `[`; all unshifted Option arrows remain free for the host and text editing. `next-change` / `prev-change` default to `}` / `{`, step each added or removed row, and cross directly to the nearest changed row in the adjacent file. They are inert outside Changes source, in previews and notices, and while selecting or composing.
