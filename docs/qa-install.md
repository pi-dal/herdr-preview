...
How to run a locally built Herdr Preview engine inside real Herdr panes. Follow it exactly. Every
step exists because skipping it has broken a session before.

The installed plugin lives at `~/.config/herdr/plugins/github/pi-dal.herdr-preview-<hash>/` and its
panes run `bin/herdr-preview` from that directory by absolute path. QA means swapping that one
...
2. Back up the original binary to `bin/herdr-preview.release-backup`, only if no backup exists
...
4. Run `bin/herdr-preview --resolve-plugin-config` and require exit 0 before touching any pane.
5. Print the pids of running panes still on the old binary.

## Rule 1: never overwrite the binary in place

`cp target/release/herdr-preview <plugin>/bin/herdr-preview` onto an existing file keeps the old
...
nothing. Each Preview pane must be closed and reopened. `pgrep -f herdr-preview` with start
...
(`herdr plugin action invoke close --plugin pi-dal.herdr-preview` sweeps the focused workspace's
Preview panes), but opening is not. After the swap, tell the user: press the Preview toggle in
each space you want on the new build. Do not automate it.

## Verify

- `<plugin>/bin/herdr-preview --resolve-plugin-config` exits 0 and prints the config JSON.
- After the user reopens a pane, `ps -o lstart= -p $(pgrep -f herdr-preview)` shows a start
  time later than the swap.

## Rollback

```
cd ~/.config/herdr/plugins/github/pi-dal.herdr-preview-*/bin
cp herdr-preview.release-backup herdr-preview.staging
rm herdr-preview && mv herdr-preview.staging herdr-preview
```

Then close and reopen the panes, same as any other swap. A full reinstall
(`herdr plugin install pi-dal/herdr-preview`) also restores the managed build.
